...
#' @param rePrm A logical value. If TRUE, the item intercept and logit of item guessing parameters are
#' reparameterized into the item difficulty and item guessing parameters, respectively. To estimate the IRT
#' linking coefficients in this package, the item difficulty and item guessing parameters must be used.
#' Default is TRUE.
...
#'
#' @details The \code{\link{bring.flexmirt}} was written by modifying the function \code{\link[rpf]{read.flexmirt}}
#' (Pritikin, 2018). The functions \code{\link{bring.bilog}} and \code{\link{bring.parscale}}
#' were written by modifying the functions \code{\link[plink]{read.bilog}} and \code{read.parscale}
...
#'
#' Although \code{\link{bring.flexmirt}} is able to extract mutidimensional item and ability parameter estimates,
#' this package only deals with unidimensional IRT linking and equating methods.
#'
...
#' necessary for the IRT linking. The object "full_df" is a data.frame containing the item meta data
#' in a test form (e.g., item parameters, number of categories, models). See \code{\link{preplink}}, \code{\link{test.info}},
...
#' parameter subtracted by the difficulty (or threshold) parameter for each category. See \code{\link{irtlink}} for more details
...
#' using \code{\link{bring.parscale}} and \code{\link{bring.flexmirt}}, four item parameter
#' estimate output files are included in this package.
#'
#' Among the four output files, two are from PARSCALE 4 with a file extension of ".PAR"
#' (i.e., "parscale_base.PAR" and "parscale_new.PAR") and the rest are from flexMIRT
#' with a file extension of "-prm.txt" (i.e., "flexmirt_base-prm.txt" and "flexmirt_new-prm.txt").
#'
#' For the four IRT software, the base and new test forms are mixed-format tests with 55 items
#' consisting of fifty dichotomous items following the IRT 3PL model and five polytomous items with five
#' categories following the graded response model. In each test form, there are 15 common items
#' and they are items 41, 42, 42, ..., 54, 55. The examples below show how to import those output files.
#'
#' @author Hwanggyu Lim \email{hglim83@@gmail.com}
#'
#' @seealso \code{\link{irtlink}}
#'
#' @references
...
#' scale data [Computer Program]. Chicago, IL: Scientific Software International. URL http://www.
#' ssicentral.com
#'
...
#' @export
bring.flexmirt <- function(file, type=c("par", "sco"), rePrm = TRUE, n.factor=1) {
  ...
  if(type=="PAR") {
    results <- bring.flexPrm(file, rePrm)
  }

  # When "sco" file is inserted
  if(type=="SCO") {
    results <- bring.flexSco(file, n.factor)
  }
  results
}
...
#' @importFrom utils count.fields read.delim read.fwf read.table
bring.flexPrm <- function(file, rePrm = TRUE) {

  groups <- list()

  ncol <- max(utils::count.fields(file, sep = "\t"))
  prm <- utils::read.delim(file,header=F, col.names=1:ncol)
  ...
  for (g in 1:ng) {
    ...
    for (i in 1:nrow(thisGroup)) {
      ...
      if (thisGroup[i,1] == 1) { # item
        ...
        if (thisGroup[i,5] == 3) { # gpc
          ...
          if(dims == 1 & isGpc) {
            T <- matrix(0, nrow=catg, ncol=(catg-1))
            T[, 1] <- 0:(catg-1)
            for(k in 2:(catg-1)) {
              for(j in 2:(catg-1)) {
                T[k, j] <- sin( pi*(j-1)*(k-1) / (catg-1) )
              }
            }
            c.Vec <- T%*%gam
            b <- -gam[1] / a
    ...
    DF <- data.frame(id=unlist(g.label), cats=unlist(g.category),
                     model=unlist(g.model), par=pmat)
    rownames(pmat) <- g.label
    groups[[g]] <- list(id=unlist(g.label), factor=unlist(g.dim), cats=unlist(g.category),
                        model=unlist(g.model), par=pmat, full_df=DF, mean=g.dist$Mean, cov=g.dist$Cov)
